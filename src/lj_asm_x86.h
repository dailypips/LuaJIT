...
static Reg asm_fuseload(ASMState *as, IRRef ref, RegSet allow)
{
  ...
  if (ir->o == IR_KNUM) {
	...
	as->mrm.ofs = op1ofs + ((ir->op2&IRSLOAD_FRAME)?4:0);
	as->mrm.idx = RID_NONE;
	return RID_MRM;
      }
...
static void asm_retf(ASMState *as, IRIns *ir)
{
  ...
  emit_addptr(as, base|REX_GC64, -8*delta);
...
static void asm_ahustore(ASMState *as, IRIns *ir)
{
  ...
  if (irt_isnum(ir->t)) {
    ...
    if (!irref_isk(ir->op2)) {
      ...
      lua_assert(irt_is64(ir->t) || irt_ispri(ir->t));
      src = ra_scratch(as, allow);
      rset_clear(allow, src);
      asm_fuseahuref(as, ir->op1, allow);
      emit_mrm(as, XO_MOVto, src|REX_64, RID_MRM);
      emit_loadu64(as, src, irt_is64(ir->t) ?
	((uint64_t)irt_toitype(ir->t) << 47) | irr[1].tv.u64 :
	~((uint64_t)irt_type(ir->t) << 47));
      return;
    }
#endif
    asm_fuseahuref(as, ir->op1, allow);
...
{
  int32_t ofs = 8*((int32_t)ir->op1-1-LJ_FR2) + ((ir->op2&IRSLOAD_FRAME)?4:0);
  IRType1 t = ir->t;
  Reg base;
...
static void asm_stack_restore(ASMState *as, SnapShot *snap)
{
  ...
  for (n = 0; n < nent; n++) {
	...
	if (irt_is64(ir->t)) {
	  ...
	  emit_movmroi(as, RID_BASE, ofs+4, (irt_type(ir->t)<<15)|0x7fff);
	}
#endif
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
  ...
  for (; p < pe; p++)
    if (*(uint32_t*)(p+(LJ_64!=LJ_GC64?3:2)) == statei && p[0] == XI_MOVmi) {
      p += LJ_64 ? 11 : 10;
      break;
    }
  lua_assert(p < pe);
